...
#-------------Delivery Companies Page Functions-------------#
    ...
    def addDeliveryCompany(self):
        try:
            row = self.ui.DeliveryCompany_tableWidget.rowCount()
            self.ui.DeliveryCompany_tableWidget.insertRow(row)

            if(row > 0):
                newId = self.allDeliveryCompanies[-1].getID()
                newAccessCode = self.allDeliveryCompanies[-1].getAccessCode()
            ...
            with open("../Resources/Data/deliveryCompanies.txt", "a") as a_file:
                a_file.write("\n")
                a_file.write(info)
